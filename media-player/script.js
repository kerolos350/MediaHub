...
function fullCk() {
    if (window.innerWidth == screen.Width || window.innerHeight == screen.height) {
        fSImg.src = './media-player/svg/form-svgrepo-com.svg'; // Form icon when in fullscreen
    } else {
        fSImg.src = './media-player/svg/full-svgrepo-com.svg'; // Fullscreen icon when not in fullscreen
    }
...
function playV() {
    if (video.paused) {
        video.play();
        pPIco.src = './media-player/svg/stop-svgrepo-com.svg'; // Change to stop icon
        mobPPIco.src = './media-player/svg/stop-svgrepo-com.svg'; // Change to stop icon on mobile
        ...
        if ((video.getBoundingClientRect().left - video.getBoundingClientRect().right) > -700) {
            ...
            pPIco.src = './media-player/svg/play-svgrepo-com.svg'; // Change to play icon
            mobPPIco.src = './media-player/svg/play-svgrepo-com.svg'; // Change to play icon on mobile
        }
    }
}
...
function play() {
    if (video.paused) {
        video.play();
        pPIco.src = './media-player/svg/stop-svgrepo-com.svg';
        mobPPIco.src = './media-player/svg/stop-svgrepo-com.svg';
        if ((video.getBoundingClientRect().left - video.getBoundingClientRect().right) > -700) {
        ...
        video.pause();
        pPIco.src = './media-player/svg/play-svgrepo-com.svg';
        mobPPIco.src = './media-player/svg/play-svgrepo-com.svg';
    }
}
...
function muted() {
    if (video.muted == true) {
        ...
        muteImg.src = './media-player/svg/sound-max-svgrepo-com.svg'; // Change to unmuted icon
        volume.value = '100'; // Set volume slider to 100
        video.volume = volume.value / 100; // Update video volume
    } else {
        video.muted = true; // Mute the video
        muteImg.src = './media-player/svg/sound-mute-svgrepo-com.svg'; // Change to muted icon
...
function fS () {
    if (document.fullscreenElement) {
        ...
        fSImg.src = './media-player/svg/full-svgrepo-com.svg'; // Change to fullscreen icon
    } else {
        player.requestFullscreen(); // Enter fullscreen
        fSImg.src = './media-player/svg/form-svgrepo-com.svg'; // Change to form icon
    }
}
...
function volCh() {
    if (volume.value == 0) {
        muteImg.src = './media-player/svg/sound-mute-svgrepo-com.svg'; // Mute icon when volume is 0
        muted(); // Mute the video
    } else if (volume.value > 10 && volume.value < 50) {
        muteImg.src = './media-player/svg/sound-min-svgrepo-com.svg'; // Low volume icon
        vol((volume.value / 100)); // Set volume
    } else if (volume.value > 50) {
        muteImg.src = './media-player/svg/sound-max-svgrepo-com.svg'; // High volume icon
        vol((volume.value / 100)); // Set volume
    }
}
    ...
    onkeydown = (event) => {
        if (event.code == 'Space') {
            if (video.paused == false) {
                video.play();
                pPIco.src = './media-player/svg/stop-svgrepo-com.svg'; // Play icon
                mobPPIco.src = './media-player/svg/stop-svgrepo-com.svg'; // Mobile play icon
            } else {
                video.pause();
                pPIco.src = './media-player/svg/play-svgrepo-com.svg'; // Pause icon
                mobPPIco.src = './media-player/svg/play-svgrepo-com.svg'; // Mobile pause icon
            }
        }
        if (event.code == 'KeyM') {
